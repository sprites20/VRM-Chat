...
export function remapBVHAnimationToVRM(vrm, asset) {
  console.log(asset.animations);
  const clip = THREE.AnimationClip.findByName(asset.animations, "__MissF|__MissF")?.clone();
  if (!clip) {
    throw new Error("Animation clip 'BVH' not found.");
  }
  ...
  clip.tracks.forEach((track) => {
    ...
    const vrmBoneName = BVHVRMRigMap[ue5RigName];
